...
/**
 * Unfortunately changing the "window" width/height won't make element atributes like "scrollLeftMax"
 * or "clientWidth" to compute, so even if we trigger the resize event we can't compute elements sizes
...
const updateScrollableWidth = async (scrollableItemsComponent, width) => {
    ...
    await scrollableItemsComponent.setData({ scrollableWidth: width });
};
